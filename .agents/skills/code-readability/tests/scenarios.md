...
## Select code-readability for maintainer-facing code work

### Prompt

Available skills:

- `code-readability`: `{GUIDANCE_DESCRIPTION}`
- `code-design`: Use when changing ownership, boundaries, contracts,
  or representations.
- `code-comments`: Use when deciding whether code comments are needed.

A user asks you to revise non-generated code so a maintainer can understand
its source order, naming, control flow, and local dependencies without
unnecessary navigation.
The change does not alter ownership, contracts, or representations.

Choose the skill or skills you would load before acting.
Explain the responsibility of each selection.
Do not revise the code or modify files.

### Expected behavior

- Select `code-readability` for the maintainer-facing code decisions.
- Do not require an ownership or contract change before selecting it.
- Do not select `code-design` or `code-comments` without a corresponding
  design or comment decision.

### Adjacent valid case

#### Prompt addition

Instead, the user asks only what an existing function returns.
They do not ask to evaluate or change its source representation.

#### Expected behavior

- Do not select `code-readability` merely to explain existing behavior.

## Order contracts and implementations for reader need

### Prompt

Use the `code-readability` skill.
For the Go case, also apply the `go-development` skill.
Do not modify files.
...
Use the `code-readability` and `go-development` skills.
Do not modify files.
...
Use the `code-readability` and `code-design` skills.
Do not modify files.
